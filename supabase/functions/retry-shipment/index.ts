...
Deno.serve(async (request) => {
  ...
  try {
    ...
    console.error(error)
    return jsonResponse({ error: 'Unable to retry shipment' }, { status: 500 })
  }
})

function normalizeInput(value: unknown): RetryShipmentRequest {
  if (!value || typeof value !== 'object') {
    throw new ShipmentWorkflowError('orderNumber or orderId is required', 400)
  }

  const input = value as RetryShipmentRequest
